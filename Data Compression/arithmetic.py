...
#     p = dict(sorted([(a,p[a]) for a in p if p[a]>0], key = lambda el: el[1], reverse=True)) # eliminate zero probabilities
...
#     p = dict([(a,p[a]) for a in p if p[a]>0]) #Eliminating 0 probabilities
